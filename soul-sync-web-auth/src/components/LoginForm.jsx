...
export default function LoginForm({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onLoginError(err.message || "Invalid credentials.");
    }
  };
  ...
  const handleSocialLogin = async (provider) => {
    ...
    try {
      ...
      onLoginError(err.message || `Failed to authenticate with ${provider}.`);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            disabled={loading}
            className={`w-full pl-10 pr-4 py-2.5 rounded-xl border bg-white/50 dark:bg-slate-900/50 outline-none transition-all duration-200 text-slate-800 dark:text-slate-100 ${
              errors.email 
                ? 'border-red-400 dark:border-red-600 focus:border-red-400 focus:ring-2 focus:ring-red-100' 
        ...
        {errors.email && (
          <p className="mt-1 text-xs text-red-500 dark:text-red-400 font-medium animate-fadeIn">{errors.email}</p>
        )}
      </div>
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="••••••••"
            disabled={loading}
            className={`w-full pl-10 pr-10 py-2.5 rounded-xl border bg-white/50 dark:bg-slate-900/50 outline-none transition-all duration-200 text-slate-800 dark:text-slate-100 ${
              errors.password 
                ? 'border-red-400 dark:border-red-600 focus:border-red-400 focus:ring-2 focus:ring-red-100' 
                : 'border-slate-200 dark:border-slate-700 focus:border-brand-orange focus:ring-2 focus:ring-orange-100 dark:focus:ring-orange-950/20'
            }`}
        ...
        {errors.password && (
          <p className="mt-1 text-xs text-red-500 dark:text-red-400 font-medium animate-fadeIn">{errors.password}</p>
        )}
      </div>
      ...
      <div className="flex items-center">
        <input
          ...
          disabled={loading}
          className="h-4.5 w-4.5 rounded border-slate-300 dark:border-slate-600 text-brand-orange focus:ring-brand-orange focus:ring-offset-0 accent-brand-orange transition-colors"
        />
        <label htmlFor="remember-me" className="ml-2 text-xs font-semibold text-slate-500 dark:text-slate-400 select-none">
          Remember me
        </label>
      </div>
      ...
      <div className="flex items-center my-5">
        ...
        <span className="mx-4 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">
          or continue with
        </span>
        <div className="flex-grow border-t border-slate-200 dark:border-slate-700"></div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button
          ...
          className="flex items-center justify-center space-x-2 py-2.5 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white/70 dark:bg-slate-900/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 active:scale-[0.97] transition-all duration-150 font-bold text-xs text-slate-600 dark:text-slate-300"
        >
